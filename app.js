...
window.addEventListener('DOMContentLoaded', () => {
  ...
  function filterAttendance() {
    ...
    rows.forEach(r => {
      ...
      const checkboxTd = r.isGhost ? `<td style="text-align: center; padding: 4px;"></td>` : `<td style="text-align: center; padding: 4px;"><input type="checkbox" class="cb-att-row" data-ts="${r.ts}"></td>`;
      tr.innerHTML = `
        ${checkboxTd}
        <td>${fmtTs(r.ts)}</td>
        <td>${statusLabel}</td>
